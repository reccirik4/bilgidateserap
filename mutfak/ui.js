...
function ekranGoster(ekranId) {
    ...
    if (menuGizleEkranlar.indexOf(ekranId) !== -1) {
        altMenuGizle();
        // Floating butonu da gizle
        var floatingBtn = document.getElementById('floating-birlikte-btn');
        if (floatingBtn) floatingBtn.classList.add('gizli');
    } else if (mevcutKullanici) {
        altMenuGoster();
        // Floating butonu göster (quiz, çark, sonuç, sohbet hariç)
        var floatingGizleEkranlar = ['ekran-quiz', 'ekran-cark', 'ekran-sonuc', 'ekran-sohbet'];
        var floatingBtn = document.getElementById('floating-birlikte-btn');
        if (floatingBtn) {
            if (floatingGizleEkranlar.indexOf(ekranId) !== -1) {
                floatingBtn.classList.add('gizli');
            } else {
                floatingBtn.classList.remove('gizli');
            }
        }
    }
